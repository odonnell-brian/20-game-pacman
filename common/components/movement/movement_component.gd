...
func try_move(direction: Vector2i) -> bool:
	...
	if move_in_progress or direction == Vector2i.ZERO or not Globals.tile_manager.is_tile_movable(target_tile, ignore_doors):
		return false
...
func do_move(target_tile: Vector2i) -> void:
	...
	move_tween.tween_property(get_parent(), "global_position", TileManager.get_tile_center_point(target_tile), (time_to_move / speed_multiplier))
